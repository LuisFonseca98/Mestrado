...
% 
% image_gradient_sobel.m
% Função que ilustra a aplicação do método de gradiente (Sobel).
 
function image_gradient_sobel()
...
% Imagens.
figure(1); set(gcf,'Name', 'Sobel');
subplot(121);  imshow(I); title(' Original' ); colormap('gray'); 
subplot(122);  imshow(Ir); axis tight; title(' Sobel ' ); 

print '-dpng' 'sobel1.png'

wx = [ -1 0; 0 1 ];
Ir1 = filter2(wx,I);
wy = [ 0 -1; 1 0 ];
Ir2 = filter2(wy,I);
figure(2); set(gcf,'Name', 'Sobel');
